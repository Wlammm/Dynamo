...
float3 EvaluateAmbience(TextureCube lysBurleyCube, float3 vN, float3 org_normal, float3 to_cam, float perceptualRoughness, float ao, float3 dfcol, float3 spccol)
{
    ...
    float3 specRad = lysBurleyCube.SampleLevel(myDefaultSampler, vR, l).xyz;
    float3 diffRad = lysBurleyCube.SampleLevel(myDefaultSampler, vN, (float) (nrBrdfMips - 1)).xyz;
...
float3 EvaluateDirectionalLight(float3 albedoColor, float3 specularColor, float3 normal, float roughness, float3 lightColor, float3 lightDir, float3 viewDir)
{
    ...
    return (lightColor * lambert * (cDiff * (1.0f - cSpec) + cSpec) * PI);
}
	...
	float3 pixelPos)
{
    ...
    return lightColor * lightIntensity * attenuation * ((cDiff * (1.0f - cSpec) + cSpec) * PI);
}

float3 EvaluateSpotLight(float3 albedoColor, float3 specularColor, float3 normal, float roughness,
	float3 lightColor, float lightIntensity, float lightRange, float3 lightPos, float3 lightDir,
	float outerAngle, float innerAngle, float3 viewDir, float3 pixelPos)
{
    ...
    return (lightColor * lightIntensity * lambert * finalAttenuation * ((cDiff * (1.0f - cSpec) + cSpec) * PI));
}