...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			SetNextWindowPos(ImVec2(vp_size.x - NOTIFY_PADDING_X + viewPortPosition.x, vp_size.y - NOTIFY_PADDING_Y - height + viewPortPosition.y), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
